...
void sendLuaParams()
{
  uint8_t luaParams[] = {0xFF,
                         ...
                         (uint8_t)LUA_VERSION,
                         (uint8_t)config.GetFanSwitch()};

  crsf.sendLUAresponse(luaParams, 11);
}
...
void HandleUpdateParameter()
{
  ...
  case 5:
    Serial.print("Request FanSwitch: ");
    Serial.println(crsf.ParameterUpdateData[1]);
    config.SetFanSwitch(crsf.ParameterUpdateData[1]);
    break;
...
static void ConfigChangeCommit()
{
  ...
  config.Commit();
    ...
    digitalWrite(GPIO_PIN_FAN_EN, config.GetFanSwitch() ? HIGH : LOW);
  #endif
